            ...
            Option number: '''))
                ...
                if y not in [1,2,3,4,5,6]:
                    ...
                    for i in range(len(info)):                     #data stored as individual lists in 'info'
                        for j in range(len(info[i])):
                            print('|',info[i][j],' '*(max[j]-(len(str(info[i][j])))),'|',end='',sep='')      #printing each record column wise, spaces multiplied by the difference of length of the maximum record in that column and the length of that record     
                        print()
                    print('+','-'*(sum+8),'+',sep='')
